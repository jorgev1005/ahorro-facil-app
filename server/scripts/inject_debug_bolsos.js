...
console.log('🚀 INJECTING DEBUG LOGS INTO BOLSOS ROUTE...');

conn.on('ready', () => {
    console.log('✅ Connected.');

    // Debug version of server/src/routes/bolsos.js
    const scriptContent = `
...
router.get('/', protect, async (req, res) => {
    console.log('--- DEBUG GET /api/bolsos ---');
    console.log('Request User:', req.user ? req.user.email : 'No User', req.user ? req.user.id : 'No ID');
    console.log('Is Admin:', req.user ? req.user.isAdmin : 'Unknown');
    ...
    try {
        ...
        if (bolsos.length > 0) {
             console.log('First bolso owner:', bolsos[0].userId);
        }
...
`;
    ...
    conn.exec(cmd, (err, stream) => {
        ...
        stream.on('close', (code, signal) => {
            conn.end();
            console.log('✅ INJECTION & RESTART DONE');
