...
console.log('🚀 PUSHING DEBUG PUBLIC ROUTE...');

conn.on('ready', () => {
    console.log('✅ Connected.');

    // Correct Public Route with Debugging
    const scriptContent = `
...
router.get('/participant/:token', async (req, res) => {
    try {
        ...
        console.log('--- PUBLIC ACCESS ATTEMPT ---');
        ...
        try {
            ...
            return res.status(401).json({ error: 'Enlace inválido o expirado', details: e.message });
        }

        const { participantId } = decoded;
        console.log('Decoded Participant ID:', participantId);
        ...
        if (!participant) {
            console.error('❌ Participant not found in DB');
        ...
        if (!bolso) {
            console.error('❌ Bolso not found');
            return res.status(404).json({ error: 'Bolso no encontrado' });
        }

        console.log('✅ Access Granted for:', participant.name);
